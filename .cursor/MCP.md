...
当前已启用两个 server，定义见 [mcp.json](./mcp.json)。
...
4. **密钥** — GitHub Token、LLM Key、Context7 Key 等只放环境变量或 Cursor 用户设置，**禁止**提交到 `mcp.json` 或仓库。

## 与 Harness 的关系

```
Rules / Skills / Gates  →  约束「做什么、怎么验」
MCP                     →  查文档（Context7）、在浏览器里验（Playwright）
...
| Server | 场景 | 注意 |
|--------|------|------|
| GitHub MCP | Issue / PR / CI 状态 | Token 用 `GITHUB_TOKEN` 环境变量 |
